...
void set_up(void) {
    ...
    GPIOB->MODER &= ~(0xC000);
		GPIOB->MODER |= (0x4000);          // PB0 output (01)

    // Habilitar pull-down en PA1
    GPIOA->PUPDR &= ~(0xC);         // Limpia PUPDR[1:0]
    ...
    // Habilitar pull-down en PA1
    GPIOA->PUPDR &= ~(0xC0);         // Limpia PUPDR[1:0]
    ...
    GPIOA->PUPDR &= ~(0x300);         // Limpia PUPDR[1:0]
    ...
    GPIOB->MODER &= ~(0xC00);       // Limpiar PB5 [11:10]
    GPIOB->MODER |= (0x400);        // PB5 output (01) 
    
		// Habilitar pull-down en PA7
		GPIOA->PUPDR &= ~(0xC000);      // Limpiar PUPDR[15:14]
